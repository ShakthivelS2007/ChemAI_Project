    ...
    with torch.no_grad():
        for b in loader:
            ...
            for i in range(b.num_graphs):
                ...
                source = getattr(b, 'source', ['?'] * b.num_graphs)[i]
                print(f"\n[COMPOUND: {formula} | MATRIX INDEX #{selected_indices[i]} | SOURCE: {source}]")

                for key in REGRESSION_KEYS:
                    p_raw = preds[key]
                    mask_val = bool(b.mask_dict[key].view(b.num_graphs, -1)[i, 0])
                    if not mask_val:
                        print(f"{key.replace('_',' ').title():<25} | (no real label for this material -- skipping)")
                        continue

                    if key == "stability_window":
                        gate_prob = torch.sigmoid(preds["stability_gate"][i]).item()
                        if gate_prob < 0.5:
                            p_val = torch.zeros_like(p_raw[i])
                        else:
                            p_val = p_raw[i]
                        t_val = b.y_dict[key][i]
                        print(f"{'Stability Window':<25} | Pred: {p_val.mean().item():<12.4f} | "
                              f"Actual: {t_val.mean().item():<12.4f} | P(nonzero window): {gate_prob:.2f}")
                        continue

                    if key in LOG_SPACE_KEYS and key in stats:
                    ...
                    t_val = b.y_dict[key][i]
                    print(f"{key.replace('_', ' ').title():<25} | Pred: {p_val.mean().item():<12.4f} | Actual: {t_val.mean().item():<12.4f}")
